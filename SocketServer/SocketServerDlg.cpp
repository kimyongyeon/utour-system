...
BOOL CSocketServerDlg::OnInitDialog()
{
	...
	m_Count = 0; // 2009.12.05 KYY ADD
	memset(&FileWrite, 0x00, sizeof(FileWrite)); // 2009.12.05 KYY ADD
...
void CSocketServerDlg::AppendFileWrite(char* strtext, ...)
{
	char szTime[20];
	char buff[1024];
// 	CFile file;
// 	CString m_strFilePath = "C:\\text.txt";
// 
// 	if(file.Open(m_strFilePath,CFile::modeCreate | CFile::modeWrite | CFile::modeNoTruncate) ==0)
// 		return;
// 
// 	file.SeekToEnd();
	...
	vsprintf(buff,strtext,arglist);
	va_end(arglist);
	
	CTime time = CTime::GetCurrentTime();
	memset(szTime,0x00,sizeof(szTime));
	sprintf(szTime,"%02d:%02d:%02d =>",time.GetHour(),time.GetMinute(),time.GetSecond());
	m_file.Write(szTime,strlen(szTime));
	
	strcat(buff,"\r\n");
	m_file.Write(buff,strlen(buff));
//	file.Close();	
}
...
void CSocketServerDlg::OnThreadBegin(CSocketHandle* pSH)
{
    ...
    AppendText( _T("Server Running on: %s,%d\r\n"), strAddr, m_Count);	
}
...
void CSocketServerDlg::OnDataReceived(CSocketHandle* pSH, const BYTE* pbData, DWORD dwCount, const SockAddrIn& addr)
{
	...
	memset(&msg, 0x00, MSG_SiZE + MSG_HEADER);
	memset(&rcvData, 0x00, MSG_SiZE + MSG_HEADER);
	...
	#pragma omp parallel num_threads(2)
	{
		#pragma omp sections
		{
			#pragma omp section
			{
				for(int i=0; i<MSG_SiZE + MSG_HEADER; i++)
					rcvData[i] = pbData[i];
				
				memcpy(&FileWrite[m_Count], &rcvData, MSG_SiZE + MSG_HEADER); // 2009.12.05 KYY ADD

				m_Count++;

				if(m_Count == ROW_Count)
					memcpy(FileBuffer, FileWrite, sizeof(FileWrite));
			}
			...
			#pragma omp section 
			{
				if(m_Count == ROW_Count)
				{
					m_file.Open(m_strFilePath,CFile::modeCreate | CFile::modeWrite | CFile::modeNoTruncate);
					m_file.SeekToEnd();
					clock_t start = clock();
					...
					{
						memcpy(&msg[1], &FileBuffer[i], MSG_SiZE + MSG_HEADER);
						AppendFileWrite("%d", msg[1].msgID);
					}

					result = (double)(clock() - start) / CLOCKS_PER_SEC;
					sprintf(timetemp, "%5.2f[sec]", result);
					AppendFileWrite("%s", timetemp);

					m_file.Close();	

					AppendText("File Write\n");
					m_Count = 0;
				}
			}
		}
